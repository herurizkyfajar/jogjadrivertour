...
@section('title', 'Edit Car - Dashboard')

@section('content')
<section class="profile-dashboard">
    <div class="inner-header mb-40">
        <h3 class="title">Edit Car</h3>
        <p class="des">{{ $car->name }}</p>
    </div>
        ...
        <form action="{{ route('admin.cars.update', $car) }}" method="POST" enctype="multipart/form-data">
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:20px;">
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name', $car->name) }}" required placeholder="e.g. Avanza TSS" style="width:100%;padding:12px 16px;border:1px solid #ddd;border-radius:8px;font-size:14px;box-sizing:border-box;">
                </div>
                <div>
                    ...
                    <select name="type" required style="width:100%;padding:12px 16px;border:1px solid #ddd;border-radius:8px;font-size:14px;box-sizing:border-box;background:#fff;">
                        <option value="Regular Car" {{ old('type', $car->type) === 'Regular Car' ? 'selected' : '' }}>Regular Car</option>
                        <option value="Premium Car" {{ old('type', $car->type) === 'Premium Car' ? 'selected' : '' }}>Premium Car</option>
                    </select>
                </div>
                <div>
                    <label style="display:block;font-weight:600;font-size:14px;color:#081E2A;margin-bottom:8px;">Kapasitas Penumpang *</label>
                    <input type="number" name="passengers" value="{{ old('passengers', $car->passengers) }}" min="1" max="50" required style="width:100%;padding:12px 16px;border:1px solid #ddd;border-radius:8px;font-size:14px;box-sizing:border-box;">
                </div>
                <div>
                    <label style="display:block;font-weight:600;font-size:14px;color:#081E2A;margin-bottom:8px;">Kapasitas Bagasi *</label>
                    <input type="number" name="luggage" value="{{ old('luggage', $car->luggage) }}" min="0" max="50" required style="width:100%;padding:12px 16px;border:1px solid #ddd;border-radius:8px;font-size:14px;box-sizing:border-box;">
                </div>
                <div>
                    <label style="display:block;font-weight:600;font-size:14px;color:#081E2A;margin-bottom:8px;">Urutan Tampil</label>
                    <input type="number" name="sort_order" value="{{ old('sort_order', $car->sort_order) }}" min="0" style="width:100%;padding:12px 16px;border:1px solid #ddd;border-radius:8px;font-size:14px;box-sizing:border-box;">
                </div>
                <div>
                    ...
                    <div id="upload-area" style="border:2px dashed #ddd;border-radius:8px;padding:20px;text-align:center;cursor:pointer;transition:all 0.3s;" onclick="document.getElementById('image-input').click();" onmouseover="this.style.borderColor='#4DA528'" onmouseout="this.style.borderColor='#ddd'">
                        ...
                        @if($car->image)
                            @if(str_starts_with($car->image, 'cars/'))
                                <img id="image-preview" src="{{ asset('storage/'.$car->image) }}" alt="{{ $car->name }}" style="max-width:100%;max-height:200px;border-radius:6px;">
                            @else
                                <img id="image-preview" src="{{ asset($car->image) }}" alt="{{ $car->name }}" style="max-width:100%;max-height:200px;border-radius:6px;">
                            @endif
                            <div id="upload-placeholder" style="display:none;">
                                <p style="margin:0;color:#999;font-size:13px;">Click to change image</p>
            ...
            <div style="margin-top:20px;">
                ...
                <textarea name="description" rows="3" placeholder="Description of the vehicle..." style="width:100%;padding:12px 16px;border:1px solid #ddd;border-radius:8px;font-size:14px;box-sizing:border-box;resize:vertical;">{{ old('description', $car->description) }}</textarea>
            </div>
            ...
            <div style="margin-top:20px;display:flex;gap:20px;align-items:center;">
                <label style="display:flex;align-items:center;gap:8px;cursor:pointer;font-weight:600;font-size:14px;color:#081E2A;">
                    <input type="checkbox" name="is_active" value="1" {{ old('is_active', $car->is_active) ? 'checked' : '' }} style="width:18px;height:18px;accent-color:#4DA528;">
                    Active (tampilkan di website)
                </label>
            </div>

            <div style="margin-top:30px;display:flex;gap:12px;">
                <button type="submit" style="background:#4DA528;color:#fff;padding:12px 30px;border-radius:8px;border:none;cursor:pointer;font-size:15px;font-weight:600;">Update Car</button>
